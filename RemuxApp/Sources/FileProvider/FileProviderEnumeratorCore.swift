...
actor FileProviderEnumeratorCore {
    ...
    func enumerateItems() async throws -> FileProviderEnumerationPage {
        ...
        case .directory(let directory):
            let items = try await refresh(directory: directory)
    ...
    ) async throws -> [FileProviderIdentifiedItem] {
        try await coordinator.perform {
            let remoteItems = try await self.service.list(directory: directory)
            try Task.checkCancellation()
            let identifiedItems = try await self.snapshots.record(
                directory: directory,
                items: remoteItems
            )
            try Task.checkCancellation()
